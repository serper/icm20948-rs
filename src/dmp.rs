...
use crate::compass;
use crate::device::{
    Icm20948, Icm20948Error, INV_NEEDS_ACCEL_MASK, INV_NEEDS_COMPASS_MASK, INV_NEEDS_GYRO_MASK,
};
    ...
    D: DelayNs,
{
    ...
    pub fn initialize(&mut self) -> Result<(), Icm20948Error> {
        ...
        self.device
            .write_reg::<bank3::Bank>(bank3::I2C_MST_ODR_CONFIG, mst_odr_config)?;
        ...
        self.device
            .write_reg::<bank0::Bank>(bank0::PWR_MGMT_2, 0x40)?;
        ...
        self.device
            .write_mems_reg::<bank0::Bank>(bank0::FIFO_EN_1, 0x00)?;
        self.device
            .write_mems_reg::<bank0::Bank>(bank0::FIFO_EN_2, 0x00)?;
        self.device
            .modify_reg::<bank0::Bank, _>(bank0::INT_ENABLE_1, |val| {
                val & !bits::INT_RAW_DATA_RDY_EN
            })?;
        ...
        self.device.write_mems_regs::<bank2::Bank>(
            bank2::PRGM_START_ADDRH,
    ...
    fn i2c_controller_configure_peripheral(
        ...
        self.device
            .write_reg::<bank3::Bank>(periph_addr_reg, addr_val)?;
        ...
        if !read_not_write && slave_num <= 3 {
            if let Some(data_out) = data_out {
                self.device
                    .write_reg::<bank3::Bank>(periph_do_reg, data_out)?;
            } else {
        ...
        self.device
            .write_reg::<bank3::Bank>(periph_reg_reg, reg_addr)?;
        ...
        self.device
            .write_reg::<bank3::Bank>(periph_ctrl_reg, ctrl_val)?;
    ...
    pub fn enable_dmp(&mut self, enable: bool) -> Result<(), Icm20948Error> {
        ...
        self.device
            .write_mems_reg::<bank0::Bank>(bank0::USER_CTRL, user_ctrl)?;
        Ok(())
    }
    ...
    pub fn reset_dmp(&mut self) -> Result<(), Icm20948Error> {
        self.device
            .modify_mems_reg::<bank0::Bank, _>(bank0::USER_CTRL, |val| val | bits::DMP_RST)?;
    ...
    pub fn reset_control_registers(&mut self) -> Result<(), Icm20948Error> {
        ...
        self.device
            .write_mems(dmp::data_output_control::DATA_OUT_CTL1, &zeros)?;
        self.device
            .write_mems(dmp::data_output_control::DATA_OUT_CTL2, &zeros)?;
        self.device
            .write_mems(dmp::data_output_control::DATA_INTR_CTL, &zeros)?;
        self.device
            .write_mems(dmp::data_output_control::MOTION_EVENT_CTL, &zeros)?;
        self.device
            .write_mems(dmp::data_output_control::DATA_RDY_STATUS, &zeros)?;
    ...
    pub fn enable_dmp_sensor(&mut self, sensor: Sensor, enable: bool) -> Result<(), Icm20948Error> {
        ...
        for i in 0..32 {
            ...
            if ((self.device.base_state.enabled_sensors_0 & android_sensor_as_bit_mask)
        ...
        if (delta & dmp::output_mask::GYRO_CALIBR as u16) != 0
            || (delta & dmp::output_mask::GYRO as u16) != 0
        {
        ...
        // DATA_INTR_CTL uses the same bit map as DATA_OUT_CTL1 (not DATA_RDY_STATUS).
        self.set_data_interrupt_control(delta as u32)?;
    ...
    pub fn set_data_interrupt_control(&mut self, interrupt_ctl: u32) -> Result<(), Icm20948Error> {
        ...
        self.device
            .write_mems(dmp::data_output_control::DATA_INTR_CTL, &bytes)
    }
    ...
    pub fn set_fifo_watermark(&mut self, fifo_wm: u16) -> Result<(), Icm20948Error> {
        ...
        self.device
            .write_mems(dmp::data_output_control::FIFO_WATERMARK, &bytes)
    }
    ...
    pub fn set_data_rdy_status(&mut self, data_rdy: u16) -> Result<(), Icm20948Error> {
        ...
        self.device
            .write_mems(dmp::data_output_control::DATA_RDY_STATUS, &bytes)
    }
    ...
    /// Get sensor ODR divider value.
    pub fn get_sensor_rate(&mut self, sensor: OdrSensor) -> Result<i16, Icm20948Error> {
        let odr_addr = match sensor {
            OdrSensor::Accel => dmp::odr::ACCEL,
            OdrSensor::Gyro => dmp::odr::GYRO,
            OdrSensor::Cpass => dmp::odr::CPASS,
    ...
    pub fn set_accel_cal_params(&mut self, accel_cal: &[i32; 3]) -> Result<(), Icm20948Error> {
        ...
        self.device
            .write_mems(dmp::scale::ACCEL_ALPHA_VAR, &alpha_var_bytes)?;
        self.device
            .write_mems(dmp::scale::ACCEL_A_VAR, &a_var_bytes)?;

        // Only write lower 16 bits of div
        let div_short_bytes = [(div_bytes[2]), (div_bytes[3])];
        self.device
            .write_mems(dmp::scale::ACCEL_CAL_RATE, &div_short_bytes)?;

        Ok(())
    }

    /// Set compass cal parameters
    pub fn set_compass_cal_params(
        &mut self,
        time_buffer: u16,
        radius_thresh: Option<u32>,
    ) -> Result<(), Icm20948Error> {
        let time_buffer_bytes = time_buffer.to_be_bytes();
        self.device
            .write_mems(dmp::compass_cal::TIME_BUFFER, &time_buffer_bytes)?;

        if radius_thresh.is_some() {
            let radius_thresh_bytes = radius_thresh.map_or([0u8; 4], |r| r.to_be_bytes());
            self.device.write_mems(
                dmp::compass_cal::RADIUS_3D_THRESH_ANOMALY,
                &radius_thresh_bytes,
            )?;
        }
    ...
    pub fn set_b2s_matrix(&mut self, b2s_mtx: &[u32; 9]) -> Result<(), Icm20948Error> {
        let mtx_addrs = [
            dmp::b2s_mtx::B2S_MTX_00,
            dmp::b2s_mtx::B2S_MTX_01,
            dmp::b2s_mtx::B2S_MTX_02,
            dmp::b2s_mtx::B2S_MTX_10,
            dmp::b2s_mtx::B2S_MTX_11,
            dmp::b2s_mtx::B2S_MTX_12,
            dmp::b2s_mtx::B2S_MTX_20,
            dmp::b2s_mtx::B2S_MTX_21,
            dmp::b2s_mtx::B2S_MTX_22,
        ];
    ...
    pub fn reset_bac_states(&mut self) -> Result<(), Icm20948Error> {
        ...
        let i32_addrs = [
            dmp::bac_state::STATE,
            dmp::bac_state::STATE_PREV,
            dmp::bac_state::ACT_ON,
            dmp::bac_state::ACT_OFF,
            dmp::bac_state::STILL_S_F,
            dmp::bac_state::RUN_S_F,
            dmp::bac_state::DRIVE_S_F,
            dmp::bac_state::WALK_S_F,
            dmp::bac_state::SMD_S_F,
            dmp::bac_state::BIKE_S_F,
            dmp::bac_state::E1_SHORT,
            dmp::bac_state::E2_SHORT,
            dmp::bac_state::E3_SHORT,
            dmp::bac_state::VAR_RUN,
            dmp::bac_state::DRIVE_CONFIDENCE,
            dmp::bac_state::WALK_CONFIDENCE,
            dmp::bac_state::SMD_CONFIDENCE,
            dmp::bac_state::BIKE_CONFIDENCE,
            dmp::bac_state::STILL_CONFIDENCE,
            dmp::bac_state::RUN_CONFIDENCE,
            dmp::bac_state::MODE_CNTR,
            dmp::bac_state::STATE_T_PREV,
            dmp::bac_state::ACT_T_ON,
            dmp::bac_state::ACT_T_OFF,
            dmp::bac_state::STATE_WRDBS_PREV,
            dmp::bac_state::ACT_WRDBS_ON,
        ...
        let i16_addrs = [
            dmp::bac_state::ACT_ON_OFF,
            dmp::bac_state::PREV_ACT_ON_OFF,
            dmp::bac_state::CNTR,
        ];
    ...
    pub fn set_orientation_params(
        ...
        self.device
            .write_mems(dmp::orientation::Q0_QUAT6, &q0_bytes)?;
        self.device
            .write_mems(dmp::orientation::Q1_QUAT6, &q1_bytes)?;
        self.device
            .write_mems(dmp::orientation::Q2_QUAT6, &q2_bytes)?;
        self.device
            .write_mems(dmp::orientation::Q3_QUAT6, &q3_bytes)?;
